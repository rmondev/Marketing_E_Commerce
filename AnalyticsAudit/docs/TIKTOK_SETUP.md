...
0. **`npm run tiktok:mint` (recommended).** A built-in helper that drives the browser authorization, captures the `code`, and tries the code→token exchange across request-shape variants for you:
   ...
   > **Note:** TikTok's authorize endpoint only accepts `code_challenge_method=S256` (the helper's default). Passing `--method plain` makes TikTok reject the login with a `code_challenge_method` error *before* issuing a code — so `plain` is a dead end, not a workaround. The historical sandbox bug, if it's still present, lives in the S256 token *exchange*, which the helper probes across request-shape variants.
