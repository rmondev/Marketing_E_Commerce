...
- **No web UI.** CLI only. Reports are static files (Markdown for the rolling report, HTML for the monthly view).
...
- **No schema migration system in v0.** Schema changes require manual `DROP TABLE` per the runbook. Migrations are deferred until schema actually starts churning.
